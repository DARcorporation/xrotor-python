"""This file contains several interface functions to make communication with XROTOR easier.

Please note that these functions are NOT thread safe! This is because XROTOR uses global variables stored inside a
FORTRAN module to store internal data before, during, and after analyses. Within one instance of Python, the XROTOR
module will use the same memory locations. If you wish to run multiple analyses concurrently, you should use process
based concurrency.
"""
from math import pi
import numpy as np
...
def reinitialize():
    """Reinitialize XROTOR data to defaults."""
    flib.init()


def set_case(case):
    """Store an XROTOR case in memory in preparation for analyses."""
...
def get_performance():
    """Get performance data from XROTOR for current analysis case."""
    rho = flib.common.rho
    vel = flib.common.vel
    rad = flib.common.rad
    adv = flib.common.adv
...
def get_distributions():
    """Get the distributions of chord, twist, lift coefficient, drag coefficient, Reynolds number, and stall indicator.

    :returns
        a dictionary with the radial positions of stations, 'xi', local chord lengths, 'chord', twist angles, 'twist',
        lift coefficients, 'cl', drag coefficients, 'cd', stall indicators, 'stalled', and Reynolds numbers, 'Re'.
    """
    ii = flib.common.ii
...
def operate(rpm=None, thrust=None):
    """Run the XROTOR analysis for a given RPM or thrust.

    If thrust is given, the given value for the rpm will be ignored. One of the two has to be given.
    :param rpm      Rotational speed of the propeller in revolutions per minute
    :param thrust   Thrust of the propeller in Newton
    :returns        True if XROTOR converged, False if not
    """
    if thrust is None:
        if rpm is None:
